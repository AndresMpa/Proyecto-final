...
struct FechasDeReservacion{
    int DiasDeEstadia[100];
};
...
struct DatosHotel{
    ...
    struct FechasDeReservacion fechasDeReservacion;
...
int main(){
    ...
    //Realiza la maquetación de las vistas.
...
string getHour(){
	...
	if(atoi(Hour.c_str()) < 10) Hour = "0"+Hour;

	stringstream ss_min;
	ss_min << timePtr->tm_min;
	string Min = ss_min.str();
	if(atoi(Min.c_str()) < 10) Min = "0"+Min;

	stringstream ss_sec;
	ss_sec << timePtr->tm_sec;
	string Sec = ss_sec.str();
    if(atoi(Sec.c_str()) < 10) Sec = "0"+Sec;
...
void FechaAQuedarse(DatosHotel &datosDelHotel, int &posHotelAReservar){
    ...
    cout<<"Ingrese el dia, mes y anio en el que llegara al hotel simultaneamente: \n"<<endl;
    cout<<"Dia: ";
    cin>>dia;
    cout<<"\nMes: ";
    cin>>mes;
    ...
    int diasAQuedarse = 0;
    cout<<"Ingrese los dias que se quedara: ";
    cin>>diasAQuedarse;
    
    datosDelHotel.fechasDeReservacion.DiasDeEstadia[posHotelAReservar] = diasAQuedarse;
    ...
    cout<<"Fecha en la que saldra del hotel es: ";
...
void LoguearseComoAdministrador(DatosAdministrador &datosDelAdministrador, bool &estaRegistrado, DatosHotel &datosDelHotel){
    ...
    cout<<"Ingrese su contrasena de administrador: ";
    fflush(stdin);
...
void RegistroAdministrador(DatosAdministrador &datosDelAdministrador, bool &estaRegistrado, DatosUsuario &datosDelUsuario){
    ...
    if(!estaRegistrado){
        ...
        cout<<"\n"<<endl;
        cout<<"Ingrese la contrasena que desea: ";
        fflush(stdin);
...
void Escribir_principal(DatosHotel &datosDelHotel){
	...
	archivo<<"<input class='opcion_button' type='button' value='Tiempo de estancia' id='fecha'>";
	...
	archivo<<"</div>";
	archivo<<"<input id='menu' class='opcion_button' type='button' value='Menu'>";
	...
	archivo<<"<input type='button' class='opcion_button' value='Wifi' id='wifi' onclick='seleccionado(adicional=1)'>";
	archivo<<"<input type='button' class='opcion_button' value='Parqueadero' id='parqueadero'  onclick='seleccionado(adicional=2)'>";
	archivo<<"<input type='button' class='opcion_button' value='Gimnasio' id='gimnasio'  onclick='seleccionado(adicional=3)'>";
	archivo<<"<input type='button' class='opcion_button' value='Restaurante' id='restaurante'  onclick='seleccionado(adicional=4)'>";
	archivo<<"<input type='button' class='opcion_button' value='Piscina' id='piscina'  onclick='seleccionado(adicional=5)'>";
	archivo<<"<input type='button' class='opcion_button' value='Spa' id='spa'  onclick='seleccionado(adicional=6)'>";
	archivo<<"<input type='button' class='opcion_button' value='Sauna' id='sauna'  onclick='seleccionado(adicional=7)'>";
	...
	archivo<<"<img src='https://st-listas.20minutos.es/images/2012-08/339549/3658360_640px.jpg?1357427289' alt='Imagen' class='preView'>";
	...
	archivo<<"<input type='button' value='comprar' id='compra' onclick='enviar_compra("<<datosDelHotel.todosLosHoteles.preciosPorNocheDeHoteles[0]<<")'>";
	archivo<<"</div>";
	archivo<<"<div class='desplegar' id='desplegar_1'>";
	archivo<<"<a href='#' onclick="<<comillas<<"mostrar('#desplegar_1','#1')"<<comillas<<"><img src='Imagenes/desplegar.png' alt='Comentarios'></a>";
	archivo<<"</div>";
	archivo<<"</article>";
	...
	archivo<<"<textarea name='new_comments' id='nuevo_comentario' cols='30' rows='10' minlength='50' placeholder='Aqui puedes anadir un comentario'></textarea>";
	archivo<<"<input type='button' name='send' value='Anadir' id='anadir_comentario'>";
	archivo<<"</form>";
	...
	archivo<<"<cite id='autores'>Cristian Andres Grajales Perez & Andres Manuel Prieto Alvarez</cite>";
	archivo<<"</footer>";
...
void Escribir_principal_del_admi(DatosHotel &datosDelHotel){
	...
	archivo.open("Pagina_principal_para_el_admi.html", ios::out);
	if(archivo.fail()){
	    cout<<"No se pudo leer el archivo, error fatal";
	    exit(1);
	}
	...
	archivo<<"<div id='search_div'>";
	archivo<<"<input type='text' placeholder='Buscar' name='Buscar' id='search'>";
	archivo<<"<input type='button' class='opcion_button' value='Buscar' id='button_buscar' onclick='buscador()'>";
	...
	archivo<<"<div id='dates'>";
	archivo<<"<input type='button' class='opcion_button' value='Tiempo de estancia' id='fecha'>";
	archivo<<"<div id='banner_fecha' class='banners'>";
	...
	archivo<<"<article class='galeria'>";
	archivo<<"<img src='https://goo.gl/maps/hxJcrHy1dYt' alt='Imagen' class='preView'>";
	archivo<<"<div class='title'>";
	archivo<<"<h2 class='refence'><a href='https://goo.gl/maps/hxJcrHy1dYt' class='reference' target='_blank'>"<<datosDelHotel.todosLosHoteles.nombresDeHoteles[0]<<"</a></h2>";
	archivo<<"</div>";
...
void Escribir_terminos_y_condiciones(DatosHotel &datosDelHotel){
	...
	archivo<<"<br><br>";
	archivo<<"1) De acuerdo a las especificaciones tecnicas indicadas para cada producto.";
	archivo<<"<br>";
	archivo<<"2) En condiciones ambientales acorde con las especificaciones indicadas por el fabricante.";
	archivo<<"<br>";
	archivo<<"3) En uso especifico para la funcion con que fue disenado de fabrica.";
	archivo<<"<br>";
	archivo<<"4) En condiciones de operacion electricas acorde con las especificaciones y tolerancias indicadas.";
	archivo<<"<br><br><br>";
...
void Escribir_factura(DatosHotel &datosDelHotel, int posHotelAReservar){
	...
	archivo<<"<p id='stay'>"<<datosDelHotel.fechasDeReservacion.DiasDeEstadia[posHotelAReservar]<<"</p><br>";
	archivo<<"<label for='Precio_por_noche'>Precio por noche</label>";
	/*Precio por noche*/
	...
	archivo<<"<input type='button' value='Presiona para obtener costo' id=pay onclick='obtener_costo("<<comillas<<""<<datosDelHotel.todosLosHoteles.preciosTotales[posHotelAReservar]<<""<<comillas<<")'>";
	archivo<<"</form>";
